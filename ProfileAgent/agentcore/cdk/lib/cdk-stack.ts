...
export class AgentCoreStack extends Stack {
  ...
  private createRuntimeApi(spec: AgentCoreProjectSpec) {
    for (const [agentName, environment] of this.application.environments) {
      const proxy = new nodejs.NodejsFunction(this, `${toCdkId(agentName)}AgentCoreProxy`, {
        entry: path.join(process.cwd(), 'lambda', 'agentcore-proxy.ts'),
        handler: 'handler',
      ...
      new CfnOutput(this, `${toCdkId(agentName)}AgentApiUrl`, {
        description: `Public API Gateway URL for ${agentName}`,
        value: api.apiEndpoint,
      });

      new CfnOutput(this, `${toCdkId(agentName)}AgentChatUrl`, {
        description: `Chat endpoint for ${agentName}`,
        value: `${api.apiEndpoint}/bedrock/chat`,
      });
    }
  }
}
